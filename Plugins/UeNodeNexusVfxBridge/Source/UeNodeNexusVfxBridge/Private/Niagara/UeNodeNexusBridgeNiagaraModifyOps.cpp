...
TSharedPtr<FJsonObject> HandleNiagaraMaterialsGet(const FString& Operation, const FString& RequestId, const TSharedPtr<FJsonObject>& Payload)
{
    ...
    TSharedPtr<FJsonObject> Data = NiagaraListFormats::BuildMaterialsListData(System, Format);

    TSharedPtr<FJsonObject> Response = MakeEnvelope(Operation, RequestId, true);
    Response->SetObjectField(TEXT("data"), Data);
    TArray<TSharedPtr<FJsonValue>> Warnings;
    AppendNiagaraEmptySystemWarning(System, Warnings);
    if (Data->GetNumberField(TEXT("count")) == 0)
    {
...
TSharedPtr<FJsonObject> HandleNiagaraMaterialsSet(const FString& Operation, const FString& RequestId, const TSharedPtr<FJsonObject>& Payload)
{
    ...
    if (Planned == 0)
    {
        Warnings.Add(MakeShared<FJsonValueObject>(MakeNiagaraBoundaryWarning(
            ...
            TEXT("No matching Niagara renderers were found for material assignment."))));
    }
    if (Warnings.Num() > 0)
    {
        Response->SetArrayField(TEXT("warnings"), Warnings);
