...
UE Node Nexus MCP 是面向 Unreal Engine 的 AI 编辑器桥接工具，将资产查询、材质与蓝图编辑、关卡操作、Niagara 和编译诊断统一到 7 个 MCP 工具中。操作能力、参数 schema 与调用示例均可按需查询，便于 AI 理解当前编辑器并执行具体任务。

项目以 **可审阅的文本资产编辑** 为核心：将资产映射为 `.nexus` 文件，修改后先校验、预览差异，再按依赖顺序提交，由 UE 完成编译、保存和回读。同步层提供冲突检测、失败保护与恢复重试；大型查询按需返回摘要和数据句柄，控制 AI 工作流的上下文占用。
