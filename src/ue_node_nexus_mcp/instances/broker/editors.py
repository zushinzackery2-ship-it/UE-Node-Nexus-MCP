...
class Editors:
    ...
    def pin(self, _client, payload: dict) -> dict:
        ...
        require(isinstance(seconds, (float, int)) and 0 < seconds <= 3600 and bool(payload.get("reason")),
                "invalid_pin", "pin requires a reason and a duration between 0 and 3600 seconds")
        with service.lock:
            item = service.instance(payload["instance_id"])
            require(item["state"] in ("READY", "IDLE", "BLOCKED"), "instance_unavailable", "pin requires a ready editor")
            if not payload.get("dry_run", True):
                ...
                item["generation"] += 1
                service.save(item)
            return dict(instance_id=item["instance_id"], pin_seconds=seconds, dry_run=payload.get("dry_run", True),
