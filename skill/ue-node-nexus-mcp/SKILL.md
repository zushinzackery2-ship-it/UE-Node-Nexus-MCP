...
- **Large blueprint sub-graph drill** → `graph_snapshot_get(keyword="Damage")` for node-name substring, `trace_from="Event BeginPlay", trace_depth=5` for BFS neighborhood, `node_class_filter=["CallFunction"]` for class filtering, `exec_only=true` for exec-pin-only wires. Response includes `filter_stats` (total/matched/included nodes). Blueprint responses always include `available_graphs` (name + node count) — use it to pick `graph_name` instead of guessing
